...
export const createNumbersArray = (currentPage: number, pagesCount: number, isMobile?: boolean) => {
  const pages = [];
  let maxVisiblePage = isMobile ? MAX_VISIBLE_PAGE_MOBILE : MAX_VISIBLE_PAGE;
  ...
  if (isMobile && (endPage - startPage + 1) > MAX_VISIBLE_PAGE_MOBILE) {
